<x-app-layout>
    <div class="max-w-full mx-auto mt-8 bg-white px-10 py-6 rounded-md shadow-md">
        <h1 class="text-2xl font-semibold mb-4 border-b pb-2">Create New Verb to Pronoun Mapping</h1>
        <form method="POST" action="{{ route('pv-mappings.store') }}">
            @csrf
            <div class="mb-4">
                <label for="verb" class="block text-sm font-medium text-gray-700">Verb:</label>
                <input type="text" name="verb" id="verb" class="mt-1 p-2 block w-full border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500" required>
            </div>
            <div class="mb-4">
                        ...
                        <div class="grid grid-cols-2 gap-2 p-2">
                            @foreach ($pronounOptions as $id => $pronoun)
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="pronouns[]" value="{{ $id }}" class="form-checkbox h-5 w-5 text-indigo-600">
            ...
            <div class="mb-4">
                ...
                <div id="selectedPronounsContainer" class="flex flex-wrap gap-2"></div>
            </div>
            <div>
